...
╰╯╱╱╰╯╱╰┻━━━┻━━━┻━━━┻━━━┻━━━┻╯╰━╯
        ...
        {m} ☯︎ {k} ☯︎ {h} ☯︎ {sir} Recod: #Bjorka-id {x}{m} ☯︎ {k} ☯︎ {h} ☯︎ {x} ''')
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
...
def login_lagi334():
	try:
		os.system('clear')
		banner()
		cetak(nel('\t©©© Saran Ektensi : [red]Cookiedough[green] ©©©'))
...
def dump_massal():
	try:
		...
		exit()
	try:
		jum = int(input('>> Mau Berapa Id Asu ? : '))
	except ValueError:
		print('>> Masukkan Angka Anjing, Malah Huruff ')
		exit()
	if jum<1 or jum>100:
		print('>> Gagal Dump Id Asu ')
		exit()
	ses=requests.Session()
	yz = 0
	...
	try:
		...
		print(f'{x}')
		print('>> Sinyal Lo Jelek Bangsat ')
		back()
	except (KeyError,IOError):
		print(f'>>{k} Id Tidak Public {x}')
		time.sleep(3)
		back()
#-------------------[ CRACK-PENGIKUT ]----------------#
def dump_pengikut():
...
if __name__=='__main__':
	...
	try:os.system('pip install play-audio')
	except:pass
	try:os.system('clear')
	except:pass
	alvino_xy(f'\n\t{x}——> {h}Assalamualaikum Kawan Stay Halal Brader {x}')
	time.sleep(0)
	login()
